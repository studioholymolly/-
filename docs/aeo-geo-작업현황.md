...
| 필요한 값 | 환경변수 | 왜 필요한가 |
|---|---|---|
| 스튜디오 주소 | `NEXT_PUBLIC_STUDIO_ADDRESS_STREET`, `_LOCALITY` | 지역 질의("서울 제품 촬영") 후보군에 들어가려면 필수 |
| 대표 전화 | `NEXT_PUBLIC_STUDIO_PHONE` | 위와 동일 |
| 네이버 플레이스 주소 | `NEXT_PUBLIC_NAVER_PLACE_URL` | 엔티티 연결 |

지금은 값이 비어 있으면 구조화 데이터에서 **자동으로 빠집니다.** 없는 정보를 지어내는 것보다 생략이 안전하기 때문입니다.

### 확인 부탁드릴 내용

1. **가격 표기** — 하프데이 150만~220만 원, 원데이 300만~420만 원으로 공개했습니다. 이 구간이 실제로 밖에 내놓아도 되는 수준인지
2. **부가세 표기** — 견적표에 명시가 없어 페이지에서 부가세 언급을 뺐습니다. "부가세 별도"를 넣을지 알려주세요
3. **영상·BX 금액** — 견적표의 협의 기준 단가(브랜드 필름 등)는 "변동 가능·실장님 확인 필요" 항목이라 **금액을 공개하지 않았습니다.** 개별 협의로만 안내합니다
